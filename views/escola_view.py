...
# Tenta carregar fpdf para o PDF
try:
...
def registrar_log(usuario, acao, doc, produto, qtd):
    """Registra movimentações críticas em uma aba de auditoria"""
...
def renderizar_escola():
    ...
    # --- 2. RECEBER MATERIAIS ---
        ...
        with st.container(border=True):
            ...
            doc_ref = c2.text_input("Nº do Documento")
        ...
        if st.button("➕ Adicionar Produto"):
            st.session_state.lista_itens.append({'id': st.session_state.contador_itens, 'prod': None, 'qtd': 0.0})
            st.rerun()
    ...
    # --- 3. CORRIGIR/ADICIONAR EM NOTA (RESOLUÇÃO DA EXCLUSÃO E LOGS) ---
    elif menu == "✏️ Corrigir/Adicionar em Nota":
        st.subheader("✏️ Edição Avançada e Auditoria")
        df_mov = carregar_dados("db_movimentacoes")
        minhas = df_mov[df_mov['ID_Escola'] == id_escola].copy()
        
        # Inicializa lista de exclusão na sessão para persistência
        ...
        if not minhas.empty:
            ...
            if sel:
                ...
                trava_excl = st.sidebar.checkbox("🔓 Habilitar Exclusão Definitiva")
                
                novos_v = []
                for idx, row in itens_nota.reset_index().iterrows():
                    is_marcado = str(row['ID_Movimentacao']) in st.session_state.ids_para_excluir
                    
                    with st.container(border=True):
                        c1, c2, c3 = st.columns([3, 1, 1])
                        c1.markdown(f"**Item:** {row['Nome_Produto']}")
                        val_q = c2.number_input("Qtd", value=float(row['Quantidade']), key=f"edit_q_{idx}_{row['ID_Movimentacao']}", disabled=is_marcado)
                        
                        if trava_excl:
                            if not is_marcado:
                                if c3.button("🗑️ Marcar Exclusão", key=f"btn_ex_{idx}"):
                                    st.session_state.ids_para_excluir.append(str(row['ID_Movimentacao']))
                                    st.rerun()
                            else:
                                if c3.button("🔄 Desfazer", key=f"btn_undo_{idx}"):
                                    st.session_state.ids_para_excluir.remove(str(row['ID_Movimentacao']))
                                    st.rerun()
                        else: c3.write("🔒")
                ...
                with st.expander("➕ Inserir Produto nesta Nota"):
                ...
                if st.button("💾 SALVAR ALTERAÇÕES", type="primary", use_container_width=True):
                    ...
                    ids_nota_atual = [str(x) for x in itens_nota['ID_Movimentacao'].tolist()]
                    
                    # LOG DE EXCLUSÕES ANTES DE SALVAR
                    for mid in st.session_state.ids_para_excluir:
                        item_info = itens_nota[itens_nota['ID_Movimentacao'] == mid]
                        if not item_info.empty:
                            registrar_log(user_data['email'], "EXCLUSÃO", doc_o, item_info.iloc[0]['Nome_Produto'], item_info.iloc[0]['Quantidade'])

                    # Processamento do Banco
                    df_res = df_full[~df_full['ID_Movimentacao'].isin(ids_nota_atual)]
                    df_nov = pd.DataFrame(novos_v).drop(columns=['Nome_Produto', 'Label'], errors='ignore')
                    df_final = pd.concat([df_res, df_nov]).fillna("")
                    
                    if salvar_dados(df_final, "db_movimentacoes", modo='overwrite'):
                        st.session_state.ids_para_excluir = []
                        st.success("Banco de Dados Atualizado com Logs!"); st.rerun()
    ...
    # --- 6. RELATÓRIOS OFICIAIS (AGRUPAMENTO E LOGS) ---
        ...
        if not df_m.empty:
            ...
            for (doc, data, resp), group in df_m.sort_values('DT', ascending=False).groupby(['Documento_Ref', 'Data_Hora', 'ID_Usuario'], sort=False):
                with st.container(border=True):
                    st.markdown(f"📄 **Nota:** `{doc}` | 🗓️ **Data:** {data} | 👤 **Resp:** `{resp}`")
                    st.table(group[['Nome_Produto', 'Quantidade', 'Unidade_Medida', 'Tipo_Fluxo']])
            
            with st.expander("🕵️ Visualizar Logs de Auditoria (Exclusões/Alterações)"):
                st.dataframe(carregar_dados("db_logs"), use_container_width=True)
